...
const fetchStudentsInfo = async () => {
    ...
    var nextToken = null;
    do {
        const studentData = await API.graphql(graphqlOperation(listNewsletters, { nextToken }));
...
const updateEmailTemplate = (subject, content, logo) => {
    ...
    if (logo == true) {
        html = html + '<img src=\"https://trello-attachments.s3.amazonaws.com/5a64de93a3ce5a4385d18c69/1029x99/0b74d83d010262a761eefa2e524dfc33/logo.png\" alt=\"Wave Learning Festival\" width="300px"/>'
    }
...
const sendBulkEmails = async (recipients) => {
    ...
    emails.forEach(email => {
        try {
            ...
            let sourceEmail = "joy@wavelf.org"; //:D
                ...
                function(data) {
                    data.Status.forEach(function (status, i) {
                        ...
                        // Else, send is successful
                            ...
                            successfulMessageId[batched_destination[batch][i].Destination.ToAddresses] = (status.MessageId) 
                        }
                    });
                }).catch(
                    function(err) {
    ...
    if (unsuccessfulEmails.length > 0) {
        alert("All emails sent with some email issues (e.g. not an actual email, etc). Check console.log for more info.")
        ...
        <form>
            ...
            <Typography.Header2 fontSize="16px">
                Note: due to the database being currently down, using {"{{name}}"} 
                 in the email content would return the user's name "Wave User" rather than the name itself.
            </Typography.Header2>
            <Form.Input 
                name="subject"
            ...
            <br/>
            <input 
                type="checkbox" id="checklogo"
                name="uselogo" value="false"
